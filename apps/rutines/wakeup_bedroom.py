...
class WakeupRoutine(hass.Hass):
    def initialize(self):
        ...
        # never fires and the room stays dark (user-reported 2026-07-20).
    ...
    def _nudge_cover_if_closed(self, entity, target_pct):
        ...
        if pos is None:
            ...
            return
        if (self.closed_is_100 and int(pos) >= self.closed_threshold) or (
            (not self.closed_is_100) and int(pos) <= (100 - self.closed_threshold)
        ):
            self._set_cover_position(entity, target_pct)
